...
void halrf_thermal_cmd(struct rf_info *rf, char input[][16], u32 *_used,
			 char *output, u32 *_out_len)
{
	char *cmd[2] = {"-h", "set"};
	u32 val = 0, val1 = 0, val2 = 0;
	u8 i;

	if (_os_strcmp(input[1], cmd[0]) == 0) {	/*-h*/
		for (i = 0; i < 2; i++) {
	...
	} else if (_os_strcmp(input[1], cmd[1]) == 0) {	/*set*/
		_os_sscanf(input[2], "%d", &val);
		_os_sscanf(input[3], "%d", &val1);
		_os_sscanf(input[4], "%d", &val2);

		rf->is_set_thermal = (u8) val;
		rf->thermal_test[RF_PATH_A] = (u8) val1;
		rf->thermal_test[RF_PATH_B] = (u8) val2;

		RF_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
			 "echo rf ther is_set_thermal thermal_test_A thermal_test_B\n");

		RF_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
			 "Set thermal is_set_thermal=%d   thermal_test_A=%d   thermal_test_B=%d\n",
			 rf->is_set_thermal, rf->thermal_test[RF_PATH_A], rf->thermal_test[RF_PATH_B]);
	} else {
		RF_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
			 "Thermal A: %d\n", halrf_get_thermal(rf, RF_PATH_A));
		RF_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
