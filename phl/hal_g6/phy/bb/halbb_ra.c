...
{
	if (bb->bb_80211spec == BB_AX_IC)
		return (enum bb_mode_type)((rate_idx & 0x180) >> 7);
	else
		return (enum bb_mode_type)((rate_idx & 0xf00) >> 8);
}
...
void halbb_rate_idx_parsor(struct bb_info *bb, u16 rate_idx, enum rtw_gi_ltf gi_ltf, struct bb_rate_info *ra_i)
{
	...
	ra_i->gi_ltf = gi_ltf;
	ra_i->mode = halbb_get_rate_mode(bb, rate_idx);
...
u8 halbb_rssi_lv_dec(struct bb_info *bb, u8 rssi, u8 ratr_state)
{
	...
	BB_DBG(bb, DBG_RA,
	       "curr RA level=(%d), Table_ori=[%d, %d, %d, %d, %d, %d, %d]\n",
	       ratr_state, rssi_lv_t[0], rssi_lv_t[1], rssi_lv_t[2],
	       rssi_lv_t[3], rssi_lv_t[4], rssi_lv_t[5], rssi_lv_t[6]);
	for (i = 0; i < RA_FLOOR_TABLE_SIZE; i++) {
		if (i >= (ratr_state))
			rssi_lv_t[i] += RA_FLOOR_UP_GAP;
	}
	BB_DBG(bb, DBG_RA,
	       "RSSI=(%d), Table_mod=[%d, %d, %d, %d, %d, %d, %d]\n", rssi,
	       rssi_lv_t[0], rssi_lv_t[1], rssi_lv_t[2], rssi_lv_t[3],
	       rssi_lv_t[4], rssi_lv_t[5], rssi_lv_t[6]);
	for (i = 0; i < RA_FLOOR_TABLE_SIZE; i++) {
		if (rssi < rssi_lv_t[i]) {
			new_rssi_lv = i;
			break;
	...
	if (rssi >= rssi_lv_t[RA_FLOOR_TABLE_SIZE - 1]) 
		new_rssi_lv = RA_FLOOR_TABLE_SIZE - 1;    /* to avoid new_rssi_lv=0 when rssi >= rssi_lv_t[6]*/
	return new_rssi_lv;
}
...
void fw_rate_idx_to_mcs_ss(u8 rate_idx, u8 *mode, u8 *mcs, u8 *ss) //Nss minus one
{
	u8 rate_tmp;
	u8 offset;
	u8 base;

	if (rate_idx >= RATE_HE1SS_MCS0) {  // HE
		offset = RATE_HE1SS_MCS0;
		base = 12;
		*mode = BB_HE_MODE;
	} else if (rate_idx >= RATE_VHT1SS_MCS0) {  // VHT
		offset = RATE_VHT1SS_MCS0;
		base = 10;
		*mode = BB_VHT_MODE;
	} else if (rate_idx >= RATE_HT_MCS0) {  // HT
		offset = RATE_HT_MCS0;
		base = 8;
		*mode = BB_HT_MODE;
	} else { // CCK/Legacy
...
void halbb_ra_rssi_setting_watchdog(struct bb_info *bb)
{
	...
	for (i = 0; i < PHL_MAX_STA_NUM; i++) {
		...
		BB_DBG(bb, DBG_RA, "macid={0x%x, 0x%x}, bcn_rssi{a,b}={%d,%d}, data_rssi{a,b}={%d,%d}, dtp_lv=%d\n",
		       ...
		       bcn_rssi_a, bcn_rssi_b, rssi_a, rssi_b, rssi_i->dtp_lv);
