				  ...
				  char *output, u32 out_len)
{
	...
	if (leaudio->bis_exist || leaudio->cis_exist) {
		CLI_PRT("\n\r %-15s : rssi:%ddBm, BIS_cnt:%d, CIS_cnt:%d",
			  ...
			  u32 input_num, char *output, u32 out_len)
{
	...
	u8 br_dbm = bt->link_info.bt_txpwr_desc.br_dbm;
	u8 le_dbm = bt->link_info.bt_txpwr_desc.le_dbm;
	...
	CLI_PRT("\n\r %-15s : profile:%s%s%s%s%s%s%s", "[profile]",
		...
		(bt_linfo->leaudio_desc.bis_exist? "BIS," : ""),
		(bt_linfo->leaudio_desc.cis_exist? "CIS," : ""));
	...
	if (bt_linfo->status.map.ble_connect ||
	    bt_linfo->leaudio_desc.bis_exist ||
	    bt_linfo->leaudio_desc.cis_exist) {
	...
	CLI_PRT("role-sw:%d, afh-sw:%d, inq:%d, pag:%d, igno_wl:%d, bt_protect:%d",
	...
	if (bt_linfo->leaudio_desc.bis_exist ||
	    bt_linfo->leaudio_desc.cis_exist) {
		CLI_PRT("; leaudio_raw_data[%02x %02x %02x %02x %02x %02x]"
		"(cnt:%d/same:%d)",
		bt->leaudio_raw_info[2], bt->leaudio_raw_info[3],
		bt->leaudio_raw_info[4], bt->leaudio_raw_info[5],
		bt->leaudio_raw_info[6], bt->leaudio_raw_info[7],
	...
	if (cx->cnt_bt[BTC_BCNT_BTTXPWR_UPDATE]) {
		CLI_PRT("\n\r %-15s : br_index:0x%x, le_index:0x%x","[bt_txpwr_lvl]",
		bt->link_info.bt_txpwr_desc.br_gain_index,
		bt->link_info.bt_txpwr_desc.le_gain_index);

		if (br_dbm > 128) {
			br_dbm = 256 - br_dbm;
			CLI_PRT(", br_dbm:-%d dBm", br_dbm);
		} else
			CLI_PRT(", br_dbm:%d dBm", br_dbm);

		if (le_dbm > 128) {
			le_dbm = 256 - le_dbm;
			CLI_PRT(", le_dbm:-%d dBm", le_dbm);
		} else
			CLI_PRT(", le_dbm:%d dBm", le_dbm);
	} else {
		CLI_PRT("\n\r %-15s : br_index:NA, le_index:NA,"
			" br_dbm:%d dBm[def], le_dbm:%d dBm[def]","[bt_txpwr_lvl]",
			bt->link_info.bt_txpwr_desc.br_dbm,
			bt->link_info.bt_txpwr_desc.le_dbm);
	}

	if (bt_linfo->status.map.ble_connect || bt_linfo->leaudio_desc.bis_exist ||
	    bt_linfo->leaudio_desc.cis_exist)
