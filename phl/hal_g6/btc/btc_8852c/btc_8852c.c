 ...
 */
u8 btc_8852c_wl_rssi_thres[BTC_WL_RSSI_THMAX] = {60, 50, 40, 30};
u8 btc_8852c_bt_rssi_thres[BTC_BT_RSSI_THMAX] = {50, 40, 30, 20};
...
const struct btc_chip chip_8852c = {
	...
	0x7, /* chip HW feature/parameter, refer to enum btc_chip_feature */
	0x7, /* desired bt_ver */
	0x080f0000, /* desired wl_fw btc ver */
	0x1, /* scoreboard version */
	0x1, /* mailbox version*/
	BTC_COEX_RTK_MODE, /* pta_mode */
	BTC_COEX_INNER, /* pta_direction */
...
void _8852c_rfe_type(struct btc_t *btc)
{
	...
	case BTC_EXT_SOC_8771:
		...
		cx->other.hw_coex = BTC_EXTSOC_INTF_PTA;
		/* use GPIO 12~15 for Ext-4-wire-PTA */
		cx->other.ext_hpta_cfg = BIT(12) | BIT(13) |
					 BIT(14) | BIT(15);
		break;
	}
}
...
void _8852c_init_ext_interface(struct btc_t *btc)
{
	struct btc_cx *cx = &btc->cx;
	u32 val;

	if (cx->other.hw_coex & BTC_EXTSOC_INTF_PTA) {
		/* 1.0xda00[29] = 1 & 0xda00[18] = 0 & 0xda00[7:5] = 0
		 * 2.0xdb00[31] = 1
		 * 0xDA00[29] : enable external BT Pinmux
		 * 0xDA00[18] : GNT_WL = GNT_WL_1 | GNT_WL_2
		 * 0xDA00[7:5] : GNT_BT = GNT_BT_1 | GNT_BT_2
		 * 0xDB00[31] : enable external PTA
		 */

		_read_cx_reg(btc, R_BTC_CFG, &val);
		val |= (B_BTC_EN | B_BTC_EXT_BT_PINMUX);
		val &= ~(B_BTC_IGN_EXT_GWL | B_BTC_IGN_EXT_GBT);

		/* Internal-PTA setup */
		_btc_io_w(btc, RTW_MAC_MAC_CMD_OFLD,
			  R_BTC_CFG, bMASKDW, val, false);

		/* if ext-PTA is 4-wire or 3-wire */
		val &= ~B_BTC_PTA_4WIRE_MODE;
		val |= cx->other.ext_pta_type << B_BTC_PTA_4WIRE_MODE_SH;

		/* External-PTA setup */
		_btc_io_w(btc, RTW_MAC_MAC_CMD_OFLD,
			  R_BTC_CFG_V2, bMASKDW, val, false);

		/* RTK cmd sample clock = 5*/
		_btc_io_w(btc, RTW_MAC_MAC_CMD_OFLD,
			  R_BTC_RTK_MODE_CFG2, bMASKB0, 0x5, false);

		/* === coex table related setup === */
		/* 0:CCA-Rx(Mixed) or 1:Idle-Rx(Isolated) */
		_btc_io_w(btc, RTW_MAC_MAC_CMD_OFLD,
			  R_BTC_COEX_WL_REQ2, BIT(0), 0x1, false);
		
		/* select external coex.table (0~3) */
		_btc_io_w(btc, RTW_MAC_MAC_CMD_OFLD,
			  R_BTC_COEX_SEL2, bMASKDW, 0x2, false);
		/* set coex. table 2 for external ZB */
		_btc_io_w(btc, RTW_MAC_MAC_CMD_OFLD,
			  R_BTC_COEX_TABLE_2_2, bMASKDW, 0x55555555, false);
		/* set break table for external ZB */
		_btc_io_w(btc, RTW_MAC_MAC_CMD_OFLD,
			  R_BTC_BREAK_TABLE_2, bMASKDW, 0xffffffff, false);
		/* enable Ext-BT counter */
		_btc_io_w(btc, RTW_MAC_MAC_CMD_OFLD,
			  R_BTC_BT_CNT_CFG2, 
