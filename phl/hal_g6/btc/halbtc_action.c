...
static void _set_rf_trx_para(struct btc_t *btc)
{
	...
	if (btc->mdinfo.ant.type == BTC_ANT_SHARED) {
		...
		if ((btc->dm.wl_btg_rx &&
			...
			if (le_audio->cis_cnt && wl->rssi_level == 4)
				dm->trx_para_level = 2;
			else
...
static void _set_halmac_tx_limit(struct btc_t *btc)
{
	...
	if (btc->dm.freerun || btc->ctrl.igno_bt || b->profile_cnt.now == 0 ||
		...
		tx_1ss_limit = 0;
	} else if (hfp->exist || hid->exist || leaudio->cis_exist) {
		enable = 1;

		if (!leaudio->cis_exist) {
			if (b->profile_cnt.now == 1 && hid->pair_cnt <= 1)
				tx_time = BTC_MAX_TX_TIME_L6;
			else if (b->profile_cnt.now == 2 && hid->pair_cnt == 1)
				tx_time = BTC_MAX_TX_TIME_L5;
			else
				tx_time = BTC_MAX_TX_TIME_L3;
		} else {
			if (leaudio->cis_cnt == 1 && hid->pair_cnt == 0)
				tx_time = BTC_MAX_TX_TIME_L4;
			else if (b->profile_cnt.now == 2 && hid->pair_cnt == 1)
				tx_time = BTC_MAX_TX_TIME_L3;
			else
	...
	for (i = 0; i < BTC_WL_MAX_ROLE_NUMBER; i++) {
		...
		tx_time = (plink->tx_time < tx_time) ? plink->tx_time : tx_time;
...
static void _set_trx_nss(struct btc_t *btc)
{
	...
	if (wl_rinfo->link_mode == BTC_WLINK_NOLINK ||
		...
		if (!a2dp->exist || a2dp->vendor_id != 0x4c) {
			tx_limit = 1;
			rx_limit = 1;
		}
	}
...
static void _set_policy(struct btc_t *btc, u16 policy_type, const char* action)
{
	...
	/* Reset E2G/EBT/E5G before entering LPS
	   to avoid these tables being modified duo to wifi fw scan */
	...
	if (btc->mdinfo.ant.type == BTC_ANT_SHARED) { /* shared-antenna */
		...
		case BTC_WSCAN_BNOSCAN: /* wl-scan + bt-idle */
			if (b->status.map.connect) {
				if (b->slave_role && !b->a2dp_desc.active) {
					btc->dm.slot_dur[CXST_W1] = 40;
					btc->dm.slot_dur[CXST_B1] = 15;
					policy_type = BTC_CXP_FIX_TDW1B1;
				} else {
...
static void _action_bt_cis(struct btc_t *btc)
{
	struct btc_wl_info *wl = &btc->cx.wl;
	struct btc_bt_info *bt = &btc->cx.bt;
	struct btc_bt_a2dp_desc *a2dp = &bt->link_info.a2dp_desc;
...
void _action_wl_2g_sta(struct btc_t *btc)
{
	...
	if (dm->freerun_chk && !dm->fddt_train_chk && !btc->cx.other.type) {
		_action_freerun(btc);
		return;
	}
	...
	if (bt_linfo->leaudio_desc.cis_exist)
	...
	case BTC_BT_HFP | BTC_BT_HID:
		...
		break;
	case BTC_BT_A2DP:
		if (dm->fddt_train_chk && !btc->cx.other.type)
