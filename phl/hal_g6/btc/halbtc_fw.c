		   ...
		   const char* action)
{
	...
	*btc_ctrl_lps = ((dm->tdma.rxflctrl == CXFLC_NULLP)? 1 : dm->lps_ctrl_scbd);
	if (dm->lps_ctrl_scbd_last != dm->lps_ctrl_scbd)
		leave_immediately = true;
	dm->lps_ctrl_scbd_last = dm->lps_ctrl_scbd;

	if (*btc_ctrl_lps == 1)
		hal_btc_notify_hal_pwr_req(btc, *btc_ctrl_lps, leave_immediately);
