...
#define PS_PROTOCAL_LEAVE_TOLERANCE 300 /* ms */
#endif
...
#ifndef MAX_CHK_PROTO_STATE_CHANGE_CNT
#define MAX_CHK_PROTO_STATE_CHANGE_CNT 3
#endif
...
rtw_hal_ps_lps_cfg(void *hal, struct rtw_hal_lps_info *lps_info)
{
	...
	hstatus = _hal_ps_lps_cfg(hal_info, lps_info);
	
	if (hstatus == RTW_HAL_STATUS_SUCCESS && !lps_info->en) {
		for (i = 0; i < MAX_CHK_PROTO_STATE_CHANGE_CNT; i++) {
			hstatus = _hal_ps_lps_chk_leave(hal_info, lps_info->macid);
...
_hal_ps_ips_chk_leave(struct hal_info_t *hal_info, u16 macid)
{
	...
	do {
		status = rtw_hal_mac_ips_chk_leave(hal_info, macid, &mac_sts);
		pass_time = phl_get_passing_time_ms(start_time);
		if (status == RTW_HAL_STATUS_SUCCESS) {
			PHL_TRACE(COMP_PHL_PS, _PHL_INFO_,
				"[HALPS], %s(): pass time = %d ms, cnt = %u.\n",
