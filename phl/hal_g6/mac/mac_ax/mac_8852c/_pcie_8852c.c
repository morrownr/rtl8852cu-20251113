...
static struct mac_ax_intf_info intf_info_def_8852c = {
	MAC_AX_BD_NORM, /* txbd_trunc_mode */
	MAC_AX_BD_NORM, /* rxbd_trunc_mode */
	MAC_AX_RXBD_SEP, /* rxbd_mode */
	MAC_AX_TAG_MULTI, /* tag_mode */
	MAC_AX_TX_BURST_V1_256B, /* tx_burst */
	MAC_AX_RX_BURST_V1_128B, /* rx_burst */
...
static struct mac_ax_pcie_ltr_param pcie_ltr_param_def_8852c = {
	...
	{MAC_AX_PCIE_ENABLE, 0}, /* ltr_dis_lat_ctrl */
	0, /* curr_ltcy */
	0, /* write */
	0, /* read */
};
...
static u16 gray_code_to_binary(u16 gray_code)
{
...
static u32 _patch_filter_out(struct mac_ax_adapter *adapter)
{
	...
	if (val32 == PCIE_GEN1_SPEED) {
		...
		val16 = gray_code_to_binary(val16);
...
u32 poll_io_idle_pcie_8852c(struct mac_ax_adapter *adapter)
{
	...
	u32 reg_trx_idle = R_AX_HAXI_INFO;
	...
	while (cnt) {
		...
		if ((!(val32 & B_IO_BUSY)) &&
		    (val32_idle & B_AX_HAXITX_IDLE) &&
		    (val32_idle & B_AX_HAXIRX_IDLE))
			break;
		cnt--;
		PLTFM_DELAY_US(PCIE_POLL_IO_IDLE_DLY_US);
	}
...
u32 pcie_aspm_frontdoor_set_8852c(struct mac_ax_adapter *adapter)
{
	...
	val32 = SET_CLR_WOR2(val32, 0x5, 0, 0xF);
...
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u16 val16, oobs_val = 0, offset_cal = 0;
	u16 val16_ana03_g1, val16_ana09_g1, val16_ana03_g2, val16_ana09_g2;
	u32 backup_aspm, ret = MACSUCCESS;
	u32 phy_offset = 0;
	u8 link_speed = 0, gen = 0;
	u8 *efuse_map = NULL;
	...
	/* offset K */
	if ((MAC_REG_R16(RAC_DIRECT_OFFSET_G1 + RAC_ANA0D * RAC_MULT) & OFFSET_CAL_MODE) |
	    (MAC_REG_R16(RAC_DIRECT_OFFSET_G2 + RAC_ANA0D * RAC_MULT) & OFFSET_CAL_MODE)) {
		for (gen = 1; gen <= 2; gen++) {
			phy_offset = (gen == 1) ? RAC_DIRECT_OFFSET_G1 :
				     RAC_DIRECT_OFFSET_G2;
		...
		val16 = MAC_REG_R16(RAC_DIRECT_OFFSET_G1 + RAC_ANA1F * RAC_MULT);
		offset_cal = GET_FIELD(val16, OFFSET_CAL);

		for (gen = 1; gen <= 2; gen++) {
			phy_offset = (gen == 1) ? RAC_DIRECT_OFFSET_G1 :
				     RAC_DIRECT_OFFSET_G2;

			/* 0x0b[8:5] = Read 0x1f[7:4] */
			val16 = MAC_REG_R16(phy_offset + RAC_ANA0B * RAC_MULT);
			val16 = SET_CLR_WORD(val16, offset_cal, MANUAL_LVL);
			MAC_REG_W16(phy_offset + RAC_ANA0B * RAC_MULT, val16);

			/* 0x0d[13]=[0] */
			val16 = MAC_REG_R16(phy_offset + RAC_ANA0D * RAC_MULT);
			MAC_REG_W16(phy_offset + RAC_ANA0D * RAC_MULT,
				    val16 & (~OFFSET_CAL_MODE));
		}
	}
	...
	if ((MAC_REG_R16(RAC_DIRECT_OFFSET_G1 + RAC_ANA09 * RAC_MULT) & BAC_OOBS_SEL) &
		...
		PLTFM_MSG_TRACE("[PCIE]PCIe OOBS is already manual mode\n");
		return MACSUCCESS;
	} else {
		PLTFM_MSG_TRACE("[PCIE]PCIe OOBS is auto mode\n");
	}
	...
	val16 = MAC_REG_R16(phy_offset + RAC_ANA1F * RAC_MULT);
	oobs_val = GET_FIELD(val16, OOBS_LEVEL);

	val16 = MAC_REG_R16(RAC_DIRECT_OFFSET_G1 + RAC_ANA03 * RAC_MULT);
	val16 = SET_CLR_WORD(val16, oobs_val, OOBS_SEN);
	MAC_REG_W16(RAC_DIRECT_OFFSET_G1 + RAC_ANA03 * RAC_MULT, val16);
	val16_ana03_g1 = val16;

	val16 = MAC_REG_R16(RAC_DIRECT_OFFSET_G1 + RAC_ANA09 * RAC_MULT) | BAC_OOBS_SEL;
	MAC_REG_W16(RAC_DIRECT_OFFSET_G1 + RAC_ANA09 * RAC_MULT, val16);
	val16_ana09_g1 = val16;

	val16 = MAC_REG_R16(RAC_DIRECT_OFFSET_G2 + RAC_ANA03 * RAC_MULT);
	val16 = SET_CLR_WORD(val16, oobs_val, OOBS_SEN);
	MAC_REG_W16(RAC_DIRECT_OFFSET_G2 + RAC_ANA03 * RAC_MULT, val16);
	val16_ana03_g2 = val16;

	val16 = MAC_REG_R16(RAC_DIRECT_OFFSET_G2 + RAC_ANA09 * RAC_MULT) | BAC_OOBS_SEL;
	MAC_REG_W16(RAC_DIRECT_OFFSET_G2 + RAC_ANA09 * RAC_MULT, val16);
	val16_ana09_g2 = val16;

	if (intf_info->oobs_efuse_en) {
		ret = mac_write_log_efuse(adapter, PCIE_OOBS_VAL_EFUSE_BYTE0_G1,
					  GET_FIELD(val16_ana03_g1, PCIE_OOBS_EFUSE_BYTE0));
		if (ret != 0)
			PLTFM_MSG_ERR("[ERR]write log efuse 0x506 fail\n");
		ret = mac_write_log_efuse(adapter, PCIE_OOBS_VAL_EFUSE_BYTE1_G1,
					  GET_FIELD(val16_ana03_g1, PCIE_OOBS_EFUSE_BYTE1));
		if (ret != 0)
			PLTFM_MSG_ERR("[ERR]write log efuse 0x507 fail\n");

		ret = mac_write_log_efuse(adapter, PCIE_OOBS_SEL_EFUSE_BYTE0_G1,
					  GET_FIELD(val16_ana09_g1, PCIE_OOBS_EFUSE_BYTE0));
		if (ret != 0)
			PLTFM_MSG_ERR("[ERR]write log efuse 0x512 fail\n");
		ret = mac_write_log_efuse(adapter, PCIE_OOBS_SEL_EFUSE_BYTE1_G1,
					  GET_FIELD(val16_ana09_g1, PCIE_OOBS_EFUSE_BYTE1));
		if (ret != 0)
			PLTFM_MSG_ERR("[ERR]write log efuse 0x513 fail\n");

		ret = mac_write_log_efuse(adapter, PCIE_OOBS_VAL_EFUSE_BYTE0_G2,
					  GET_FIELD(val16_ana03_g2, PCIE_OOBS_EFUSE_BYTE0));
		if (ret != 0)
			PLTFM_MSG_ERR("[ERR]write log efuse 0x586 fail\n");
		ret = mac_write_log_efuse(adapter, PCIE_OOBS_VAL_EFUSE_BYTE1_G2,
					  GET_FIELD(val16_ana03_g2, PCIE_OOBS_EFUSE_BYTE1));
		if (ret != 0)
			PLTFM_MSG_ERR("[ERR]write log efuse 0x587 fail\n");

		ret = mac_write_log_efuse(adapter, PCIE_OOBS_SEL_EFUSE_BYTE0_G2,
					  GET_FIELD(val16_ana09_g2, PCIE_OOBS_EFUSE_BYTE0));
		if (ret != 0)
			PLTFM_MSG_ERR("[ERR]write log efuse 0x592 fail\n");
		ret = mac_write_log_efuse(adapter, PCIE_OOBS_SEL_EFUSE_BYTE1_G2,
					  GET_FIELD(val16_ana09_g2, PCIE_OOBS_EFUSE_BYTE1));
		if (ret != 0)
			PLTFM_MSG_ERR("[ERR]write log efuse 0x593 fail\n");

		efuse_map = (u8 *)PLTFM_MALLOC(RAC_MULT);
		if (!efuse_map) {
			PLTFM_MSG_ERR("[ERR]oobs malloc efuse_map\n");
			MAC_REG_W32(R_AX_PCIE_MIX_CFG_V1, backup_aspm);
			return MACBUFALLOC;
		}
		ret = mac_read_log_efuse(adapter, PCIE_OOBS_VAL_EFUSE_BYTE0_G1, 2, efuse_map);
		if (ret != 0)
			PLTFM_MSG_ERR("[ERR]read log efuse 0x506 fail\n");
		val16 = (*(efuse_map)) | (*(efuse_map + 1) << 8);
		if (val16 != val16_ana03_g1)
			PLTFM_MSG_ERR("[ERR]efuse oobs val 0x506 compare fail\n");

		ret = mac_read_log_efuse(adapter, PCIE_OOBS_SEL_EFUSE_BYTE0_G1, 2, efuse_map);
		if (ret != 0)
			PLTFM_MSG_ERR("[ERR]read log efuse 0x512 fail\n");
		val16 = (*(efuse_map)) | (*(efuse_map + 1) << 8);
		if (val16 != val16_ana09_g1)
			PLTFM_MSG_ERR("[ERR]efuse oobs val 0x512 compare fail\n");

		ret = mac_read_log_efuse(adapter, PCIE_OOBS_VAL_EFUSE_BYTE0_G2, 2, efuse_map);
		if (ret != 0)
			PLTFM_MSG_ERR("[ERR]read log efuse 0x586 fail\n");
		val16 = (*(efuse_map)) | (*(efuse_map + 1) << 8);
		if (val16 != val16_ana03_g2)
			PLTFM_MSG_ERR("[ERR]efuse oobs val 0x586 compare fail\n");

		ret = mac_read_log_efuse(adapter, PCIE_OOBS_SEL_EFUSE_BYTE0_G2, 2, efuse_map);
		if (ret != 0)
			PLTFM_MSG_ERR("[ERR]read log efuse 0x592 fail\n");
		val16 = (*(efuse_map)) | (*(efuse_map + 1) << 8);
		if (val16 != val16_ana09_g2)
			PLTFM_MSG_ERR("[ERR]efuse oobs val 0x592 compare fail\n");

		PLTFM_FREE(efuse_map, RAC_MULT);
	}
...
u32 lv2rst_stop_dma_pcie_8852c(struct mac_ax_adapter *adapter)
{
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 ret, val32, cnt;
	...
	MAC_REG_W32(R_AX_HAXI_INIT_CFG1, MAC_REG_R32(R_AX_HAXI_INIT_CFG1) | B_AX_FLUSH_AXI_MST);
	cnt = PCIE_POLL_FLUSH_CNT;
	while (cnt) {
		val32 = MAC_REG_R32(R_AX_HAXI_INIT_CFG1);
		if ((!(val32 & B_AX_FLUSH_AXI_MST)))
			break;
		cnt--;
		PLTFM_DELAY_US(PCIE_POLL_FLUSH_DLY_US);
	}
	if (!cnt) {
		PLTFM_MSG_ERR("[ERR]SER L2 HAXIDMA flush timeout\n");
		return MACPOLLTO;
	}
...
u32 dump_pcie_ser_info_8852c(struct mac_ax_adapter *adapter)
{
	...
	if (MAC_REG_R32(R_AX_FW_DEBUG_USE_ONLY_2)) {
		...
		if (!addr) {
			PLTFM_MSG_ERR("%s get_ple_dbg_addr fail\n", __func__);
			return MACBADDR;
		}
