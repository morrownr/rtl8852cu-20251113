...
enum usb_support_ability get_usb_support_ability_8852c(struct mac_ax_adapter *adapter)
{
	...
	u2force = MAC_REG_R32(R_AX_USB_HOST_REQUEST_2_V1) &
...
u32 get_u3_perf_mode_8852c(struct mac_ax_adapter *adapter, enum mac_u3_perf_mode *perf_mode)
{
	struct mac_ax_ops *mac_ops = adapter_to_mac_ops(adapter);
	u16 val16 = 0;
	u8 rate = 0;
