	...
	void* dispr, void* priv, struct phl_msg* msg)
{
	...
	switch(MSG_EVT_ID_FIELD(msg->msg_id)) {
		...
		case MSG_EVT_SWCH_DONE:
			...
			nextmsg.rsvd[1].value = param->rsn;
		...
		u8 band_idx)
{
	...
	if (SCAN_RSN_P2P == param->rsn) {
		/* except bb */
		rtw_hal_notification_ex(phl_info->hal ,MSG_EVT_P2P_SCAN_START,
		                        false, true, true, band_idx);
	} else {
		/* except bb */
		rtw_hal_notification_ex(phl_info->hal ,MSG_EVT_SCAN_START,
		                        false, true, true, band_idx);
	}
}
	...
	void* dispr, void* priv, struct phl_msg* msg)
{
	...
	switch(MSG_EVT_ID_FIELD(msg->msg_id)) {
		case MSG_EVT_SCAN_START:
			_cmd_scan_start(phl_info, param, sctrl, band_idx);
