	...
	char input[][MAX_ARGV], u32 input_num, char *output, u32 out_len)
{
	...
	PS_CNSL(out_len, *used, output + *used, out_len - *used,
		...
		(info.assoc_gohotspot == true? "yes" : "no"),
	...
	PS_CNSL(out_len, *used, output + *used, out_len - *used,
		"init_rf_state: %s, init_rt_stop_rson: 0x%x, leave_fail_act: 0x%x, leave_fail_th: 0x%x\
		...
		(ps_cap->init_rf_state ? "off" : "on"), ps_cap->init_rt_stop_rson, ps_cap->leave_fail_act, ps_cap->leave_fail_th, phl_ps_op_mode_to_str(ps_cap->lps_en), phl_ps_pwr_lvl_to_str(phl_ps_judge_pwr_lvl(ps_cap->lps_cap, PS_MODE_LPS, true)), ps_cap->ps_pause_tx,
