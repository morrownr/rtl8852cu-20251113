...
static void _phl_dump_mac_mem(struct phl_info_t *phl_info,
                              char input[][MAX_ARGV],
                              u32 input_num,
                              char *output,
                              u32 out_len)
{
	...
	cmd = input[1];
	_get_hex_from_string(input[2], &idx);

	rtw_hal_dump_mac_mem(phl_info->hal, cmd, idx, NULL);
}
		        ...
		        u32 input_num, char *output, u32 out_len)
{
	...
	break;
#endif
	case PHL_DBG_DUMP_MAC_MEM:
		_phl_dump_mac_mem(phl_info, input, input_num, output, out_len);
	break;
