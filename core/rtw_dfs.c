...
static void rtw_dfs_ch_switch_hdl(struct dvobj_priv *dvobj, u8 band_idx)
{
	...
	/* TODO: handle of asoc STA ifaces when supporting slave with radar detect */

#ifdef CONFIG_AP_MODE
	if (ifbmp_m)
		rtw_dfs_bss_ch_switch(dvobj, band_idx, IFBMP_TO_IFLBMP(ifbmp_m));
#endif
...
static void rtw_dfs_rd_en_decision(struct dvobj_priv *dvobj, enum phl_band_idx hwband
	, struct _ADAPTER_LINK *alink, u8 mlme_act, u8 excl_ifbmp)
{
	struct rf_ctl_t *rfctl = dvobj_to_rfctl(dvobj);
	u8 band_idx;
	...
	for (band_idx = HW_BAND_0; band_idx < HW_BAND_MAX; band_idx++) {
		...
		if (alink && !alink_is_on_hwband(alink, band_idx))
			continue;
		...
		if (mlme_act == MLME_ACTION_NONE
			...
			struct _ADAPTER_LINK *alink_op = NULL;

			ifbmp = ~excl_ifbmp & ~BIT(alink->adapter->iface_id);
			rtw_mi_status_by_hwband_ifbmp(dvobj, band_idx, ifbmp, &mstate);
			rtw_mi_get_bch_setting_union_by_hwband_ifbmp(dvobj, band_idx, ifbmp, &u_band, &u_ch, &u_bw, &u_offset);
			...
			if (alink_op) {
				enum band_type band = ALINK_GET_BAND(alink_op);
				u8 ch = ALINK_GET_CH(alink_op);
				u8 bw = ALINK_GET_BW(alink_op);
				u8 offset = ALINK_GET_OFFSET(alink_op);
				...
				if (u_ch) {
					if (!rtw_is_bchbw_grouped(band, ch, bw, offset, u_band, u_ch, u_bw, u_offset)) {
						RTW_WARN(FUNC_HWBAND_FMT" "ADPT_FMT" can't sync %u,%u,%u,%u with %u,%u,%u,%u\n"
							, FUNC_HWBAND_ARG(band_idx), ADPT_ARG(alink_op->adapter)
			...
			RTW_WARN(FUNC_HWBAND_FMT" mlme_act:%u without alink\n", FUNC_HWBAND_ARG(band_idx), mlme_act);
			rtw_warn_on(1);
			continue;
		}
...
apply:
		if (alink)
			RTW_INFO(FUNC_HWBAND_FMT" needed:%d, lgd_sta_in_dfs:%d, "ADPT_FMT" mlme_act:%u, excl_ifbmp:0x%02x\n"
				, FUNC_HWBAND_ARG(band_idx), needed, lgd_sta_in_dfs, ADPT_ARG(alink->adapter), mlme_act, excl_ifbmp);
		else
		...
		if (needed && rtw_rfctl_radar_detect_supported(rfctl))
			...
			if (needed)
				rtw_dfs_bss_ch_switch(dvobj, band_idx, rtw_mi_get_ap_mesh_iflbmp_by_hwband(dvobj, band_idx));
		}
	}
}

void rtw_dfs_rd_en_dec_on_mlme_act(struct _ADAPTER_LINK *alink, u8 mlme_act, u8 excl_ifbmp)
{
	if (alink && mlme_act != MLME_ACTION_NONE)
		rtw_dfs_rd_en_decision(adapter_to_dvobj(alink->adapter), HW_BAND_MAX, alink, mlme_act, excl_ifbmp);
...
{
	rtw_dfs_rd_en_decision(dvobj, hwband, NULL, MLME_ACTION_NONE, 0);
}
...
#define DBG_CHOOSE_SHORTEST_WAITING_CH 0
	...
	enum band_type pref_band = rfctl->ch_sel_pref_band;
	u8 pref_ch = rfctl->ch_sel_pref_ch;
	...
	RTW_INFO("%s: sel_ch:%s-%u(%u) max_bw:%u e/d_flags:0x%02x/0x%02x\n", __func__
		, band_str(sel_band), sel_ch, sel_offset, max_bw, e_flags, d_flags);
	RTW_INFO("%s: cur_ch:%s-%u(%u) within_sb:%d%s%s\n", __func__
		, band_str(cur_band), cur_ch, cur_offset, within_same_band
		, by_int_info ? " int" : ""
		, mesh_only ? " mesh_only" : "");
	if (pref_band != BAND_MAX)
		RTW_INFO("%s: pref_ch:%s-%u\n", __func__, band_str(pref_band), pref_ch);
	else
		pref_ch = 0;
	...
	for (bw = CHANNEL_WIDTH_20; bw <= max_bw; bw++) {
		...
		for (i = 0; i < chset->chs_len; i++) {
			...
			if (sel_band != BAND_MAX) {
				if (band != sel_band)
					continue;
			} else if (pref_band != BAND_MAX) {
				if (band != pref_band)
					continue;
			} else if (within_same_band && cur_band != BAND_MAX && cur_band != band)
				continue;
...
exit:
	if (ch_c != 0) {
		...
		*dec_offset = offset_c;
		...
		if (sel_band == BAND_MAX) {
			if (pref_band != BAND_MAX) {
				RTW_INFO("%s: cancel pref_band and choose again", __func__);
				pref_band = BAND_MAX;
				pref_ch = 0;
				goto choose;
			}
			if (within_same_band == RTW_CHSEL_BAND_SAME_FIRST) {
...
static bool rtw_rfctl_choose_bchbw(struct rf_ctl_t *rfctl
	, enum band_type sel_band, u8 sel_ch, u8 max_bw, enum chan_offset sel_offset
...
struct bss_choose_bchbw_parms {
	enum band_type sel_band;
	u8 sel_ch;
	enum channel_width max_bw;
	enum chan_offset sel_offset;

	enum band_type cur_band;
	u8 cur_ch;
	enum chan_offset cur_offset;

	bool by_int_info;
	bool mesh_only;
};

static void rtw_get_bss_choose_bchbw_parms(struct dvobj_priv *dvobj
	, u8 hwband, u32 iflbmp, const struct bss_ch_sel_req *req
	, struct bss_choose_bchbw_parms *parms)
{
	enum band_type cur_ie_band[MAX_IFACE_LINK_BMP_SIZE];
	u8 cur_ie_ch[MAX_IFACE_LINK_BMP_SIZE];
	u8 cur_ie_bw[MAX_IFACE_LINK_BMP_SIZE];
	u8 cur_ie_offset[MAX_IFACE_LINK_BMP_SIZE];
	struct _ADAPTER_LINK *iflink;
	int i;

	/* mask out iflbmp and get cur_ie_band/ch/bw/offset */
	for (i = 0; i < MAX_IFACE_LINK_BMP_SIZE; i++) {
		if (!(iflbmp & BIT(i)))
			continue;
		iflink = rtw_get_alink_by_iflidx(dvobj, i);
		if (!iflink || hwband != ALINK_GET_HWBAND(iflink)) {
			iflbmp &= ~BIT(i);
			continue;
		}

		rtw_bss_get_bchbw(ALINK_GET_EXT_NETWORK(iflink)
			, &cur_ie_band[i], &cur_ie_ch[i]
			, &cur_ie_bw[i], &cur_ie_offset[i], 1, 1, 1, 1);
	}

	/* cur_band & cur_ch & cur_offset */
	parms->cur_band = BAND_MAX;
	parms->cur_ch = 0;
	parms->cur_offset = CHAN_OFFSET_NO_EXT;
	for (i = 0; i < MAX_IFACE_LINK_BMP_SIZE; i++) {
		if (!(iflbmp & BIT(i)))
			continue;

		if (parms->cur_band == BAND_MAX) {
			parms->cur_band = cur_ie_band[i];
			parms->cur_ch = cur_ie_ch[i];
			parms->cur_offset = cur_ie_offset[i];
		} else if (parms->cur_band != cur_ie_band[i]) {
			parms->cur_band = BAND_MAX;
			parms->cur_ch = 0;
			parms->cur_offset = CHAN_OFFSET_NO_EXT;
			break;
		} else if (parms->cur_ch != cur_ie_ch[i]) {
			parms->cur_ch = 0;
			parms->cur_offset = CHAN_OFFSET_NO_EXT;
			break;
		} else if (cur_ie_offset[i] != CHAN_OFFSET_NO_EXT) {
			if (parms->cur_offset == CHAN_OFFSET_NO_EXT)
				parms->cur_offset = cur_ie_offset[i];
			else if (parms->cur_offset != cur_ie_offset[i]) {
				parms->cur_offset = CHAN_OFFSET_NO_EXT;
				break;
			}
		}
	...
	for (i = 0; i < MAX_IFACE_LINK_BMP_SIZE; i++) {
		if (!(iflbmp & BIT(i)))
			continue;
		iflink = rtw_get_alink_by_iflidx(dvobj, i);
		if (iflink && !MLME_IS_MESH(iflink->adapter))
			break;
	}
	parms->mesh_only = i >= MAX_IFACE_LINK_BMP_SIZE;
#else
	parms->mesh_only = false;
#endif

	/* by_int_info */
	parms->by_int_info = req->ch == REQ_CH_INT_INFO;

	/* sel_band & sel_ch */
	if (req->band <= REQ_BAND_NONE || req->band >= BAND_MAX) {
		/* band is unspecified/invalid means channel is also unspecified */
		parms->sel_band = BAND_MAX;
		parms->sel_ch = 0;
	} else {
		/* band is specified and valid */
		parms->sel_band = req->band;
		parms->sel_ch = req->ch <= REQ_CH_NONE ? 0 : req->ch;
	}

	/* max_bw */
	if (req->bw > REQ_BW_NONE)
		parms->max_bw = rtw_min(req->bw, MAX_CCH_WIDTH);
	else if (req->bw == REQ_BW_MAX)
		parms->max_bw = MAX_CCH_WIDTH;
	else {
		parms->max_bw = CHANNEL_WIDTH_20;
		for (i = 0; i < MAX_IFACE_LINK_BMP_SIZE; i++) {
			if (!(iflbmp & BIT(i)))
				continue;

			if (req->bw == REQ_BW_ORI) {
				iflink = rtw_get_alink_by_iflidx(dvobj, i);
				if (!iflink)
					continue;
				if (parms->max_bw < ALINK_GET_ORI_BW(iflink))
					parms->max_bw = ALINK_GET_ORI_BW(iflink);
			} else {
				if (parms->max_bw < cur_ie_bw[i])
					parms->max_bw = cur_ie_bw[i];
			}
		}
	}

	/* sel_offset */
	if (req->offset > REQ_OFFSET_NONE) {
		switch (req->offset) {
		case CHAN_OFFSET_UPPER:
		case CHAN_OFFSET_LOWER:
			parms->sel_offset = req->offset;
			break;
		default:
			parms->sel_offset = CHAN_OFFSET_NO_EXT;
			break;
		}
	} else
		parms->sel_offset = CHAN_OFFSET_NO_EXT;
...
bool rtw_choose_bss_bchbw_iflbmp(struct dvobj_priv *dvobj
	, u8 hwband, u32 iflbmp, const struct bss_ch_sel_req *req
	, enum band_type *band, u8 *ch, u8 *bw, u8 *offset
	, const char *caller)
{
	...
	rtw_get_bss_choose_bchbw_parms(dvobj, hwband, iflbmp, req, &parms);
	return rtw_rfctl_choose_bchbw(dvobj_to_rfctl(dvobj)
		, parms.sel_band, parms.sel_ch, parms.max_bw, parms.sel_offset
		, parms.cur_band, parms.cur_ch, parms.cur_offset
		, band, ch, bw, offset
		, parms.by_int_info, parms.mesh_only, caller);
}

bool rtw_dfs_bss_ch_switch(struct dvobj_priv *dvobj, u8 hwband, u32 iflbmp)
{
	struct bss_ch_sel_req req = {REQ_BAND_NONE, REQ_CH_NONE, REQ_BW_ORI, REQ_OFFSET_NONE};
	enum band_type band;
	u8 ch, bw, offset;
	bool ch_avail;
	struct _ADAPTER_LINK *m_iflink;
...
#ifdef CONFIG_ECSA
	if (DEV_IS_ECSA_RUNNING(dvobj, hwband)) {
		RTW_INFO(FUNC_HWBAND_FMT" CSA ongoing\n", FUNC_HWBAND_ARG(hwband));
		return true;
	}
#endif

	ch_avail = rtw_choose_bss_bchbw_iflbmp(dvobj, hwband, iflbmp, &req
		, &band, &ch, &bw, &offset, __func__);
	if (!ch_avail) {
		RTW_INFO(FUNC_HWBAND_FMT" no available ch\n", FUNC_HWBAND_ARG(hwband));
		return false;
	}

	RTW_INFO(FUNC_HWBAND_FMT" %u,%u,%u,%u is selected\n", FUNC_HWBAND_ARG(hwband)
		, band, ch, bw, offset);
...
#ifdef CONFIG_ECSA
	if (rtw_hal_is_csa_support(m_iflink->adapter)) {
		struct _ADAPTER_LINK *csa_iflink = rtw_select_csa_iflink(dvobj, hwband, iflbmp);

		if (csa_iflink) {
			struct rf_ctl_t *rfctl = dvobj_to_rfctl(dvobj);
			u8 csa_mode = IS_RADAR_DETECTED(rfctl) ? CSA_STOP_TX : 0;
			u8 csa_cnt = DEFAULT_CSA_CNT;
			u8 op_class;
			bool ret;

			op_class = rtw_get_op_class_by_bchbw(band, ch, bw, offset);

			ret = rtw_hal_trigger_csa_start(csa_iflink->adapter, CSA_AP_CORE_SWITCH_CH
				, csa_mode
				, op_class, csa_cnt, band, ch, bw, offset
				, 0, 0, 0);
			if (ret)
				return true;
		}
	...
	/* change BSS's ch setting directly */
	rtw_change_bss_bchbw_cmd(m_iflink->adapter, RTW_CMDF_DIRECTLY
		, iflbmp, rtw_mi_get_lgd_sta_ifbmp_by_hwband(dvobj, hwband)
		, band, ch, bw, offset);
...
{
	rfctl->ch_sel_within_same_band = CONFIG_RTW_CHSEL_BAND_OPT;
	rfctl->ch_sel_pref_band = CONFIG_RTW_CHSEL_PREF_BAND;
	rfctl->ch_sel_pref_ch = CONFIG_RTW_CHSEL_PREF_CH;
