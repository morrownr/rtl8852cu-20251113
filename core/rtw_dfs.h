...
#define REQ_BAND_NONE	-1	/* ch selection without specific band */

#define REQ_CH_SETUP	0	/* ap/mesh setup by user space daemon (ex: wpa_supplicant/hostapd) */
#define REQ_CH_NONE	-1	/* ch selection without specific ch */
#define REQ_CH_INT_INFO	-2	/* ch selection consider interference info */

#define REQ_BW_NONE	-1	/* ch selection without specific bw */
#define REQ_BW_ORI	-2	/* ch selection with bw limit by value of setup */
#define REQ_BW_MAX	-3	/* ch selection without specific limit */

#define REQ_OFFSET_NONE	-1	/* ch selection without specific offset */

struct bss_ch_sel_req {
	s8 band; /* >= 0: enum band_type, < 0:  REQ_BAND_XXX */
	s16 ch; /* > 0: u8, <= 0:  REQ_CH_XXX */
	s8 bw; /* >= 0: enum channel_width, < 0:  REQ_BW_XXX */
	s8 offset; /* >= 0: enum chan_offset, < 0:  REQ_OFFSET_XXX */
};

#define DUMMY_BSS_CH_SEL_REQ {REQ_BAND_NONE, REQ_CH_SETUP, REQ_BW_NONE, REQ_OFFSET_NONE}
